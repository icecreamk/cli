...
const ini = require("ini"); // ini格式文件处理
const minimist = require("minimist"); // 解析命令行参数
const LRU = require("lru-cache"); // 设定缓存的最大容量

// 用于去除字符串中的 ANSI 转义序列。ANSI 转义序列通常用于在终端中添加颜色、样式或控制光标移动
...
// @exampleuser/some/path/to/resource => @exampleuser
...
class PackageManager {
  constructor({ context, forcePackageManager } = {}) {
    ...
    if (!this.bin) {
      this.bin =
        ...
        (hasYarn() ? "yarn" : hasPnpm3OrLater() ? "pnpm" : "npm");
    }
  ...
  async getRegistry(scope) {
    ...
    if (args.registry) {
      ...
      try {
        if (scope) {
          registry = (
            await execa(this.bin, ["config", "get", scope + ":registry"])
          ).stdout;
        }
  ...
  async setBinaryMirrors() {
    ...
    try {
      // chromedriver, etc.
      const binaryMirrorConfigMetadata = await this.getMetadata(
        "binary-mirror-config",
        { full: true }
      );
      const latest =
        binaryMirrorConfigMetadata["dist-tags"] &&
        binaryMirrorConfigMetadata["dist-tags"].latest;
  ...
  async upgrade(packageName) {
    ...
    if (packageNamesArray.length)
      return await this.runCommand("add", packageNamesArray);
  }
}
