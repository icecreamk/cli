...
module.exports = class Upgrader {
  ...
  async upgrade(pluginId, options) {
    ...
    if (!required) {
      throw new Error(
        `Can't find ${chalk.yellow(packageName)} in ${chalk.yellow(
          "package.json"
        )}`
      );
    }
    ...
    if (!installed) {
      throw new Error(
        `Can't find ${chalk.yellow(packageName)} in ${chalk.yellow(
          ...
          `Or to force upgrade, you can specify your current plugin version with the ${chalk.cyan(
            "--from"
          )} option`
      );
    }
    ...
    if (!/\d+\.\d+\.\d+/.test(targetVersion)) {
      if (targetVersion === "latest") {
        ...
        logWithSpinner(
          `Getting max satisfying version of ${packageName}@${options.to}`
        );
      }
      ...
      if (!options.to && options.next) {
        ...
        if (next) {
          targetVersion = semver.gte(targetVersion, next)
            ? targetVersion
            : next;
        }
      }
    ...
    if (targetVersion === installed) {
      ...
      if (newRange !== required) {
        ...
        fs.writeFileSync(
          path.resolve(this.context, "package.json"),
          JSON.stringify(this.pkg, null, 2)
        );
        log(
          `${chalk.green("✔")}  Updated version range in ${chalk.yellow(
            "package.json"
          )}`
        );
      }
    ...
    const resolvedPluginMigrator = resolveModule(
    ...
    if (resolvedPluginMigrator) {
      ...
      await execa(
        "node",
        [cliBin, "migrate", packageName, "--from", installed],
        {
          cwd: this.context,
          stdio: "inherit",
        }
    ...
    console.log(
      "  " +
        ["Name", "Installed", "Wanted", "Latest", "Command to upgrade"]
          .map((x, i) => chalk.underline(x.padEnd(pads[i])))
          .join("")
    );
    for (const p of upgradable) {
      const fields = [
